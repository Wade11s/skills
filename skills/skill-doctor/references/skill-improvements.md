...
On Pi, a skill is offered from its always-loaded `name` + `description`, then loaded when the model `read`s `SKILL.md` or the user runs `/skill:<name>`. A never-triggered skill in a failed conversation is usually a description problem on that pointer, not a missing body paragraph.
