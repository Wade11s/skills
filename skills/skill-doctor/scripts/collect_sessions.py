#!/usr/bin/env python3
"""Collect local Pi, Claude Code, Codex, and Warp sessions and skills for scoring.

Scans Pi session JSONL, Claude Code project history, Codex rollout files,
and/or Warp's local conversation databases, discovers installed skills,
...
def parse_args():
    ...
    p.add_argument(
        "--harness",
        choices=("auto", "all", "pi", "claude", "codex", "warp"),
        default="auto",
    ...
    p.add_argument(
        ...
        help="Pi config directory (default: PI_CODING_AGENT_DIR or ~/.pi/agent)",
    )
    p.add_argument(
        "--pi-sessions-dir",
        default=os.environ.get("PI_CODING_AGENT_SESSION_DIR"),
        help="Pi session storage (default: PI_CODING_AGENT_SESSION_DIR or <pi-home>/sessions)",
    )
    p.add_argument(
        "--claude-home",
...
def is_pi_skill_root(root: Path) -> bool:
    parts = [part.lower() for part in root.parts]
    if len(parts) >= 2 and parts[-2:] == [".pi", "skills"]:
        return True
...
def skill_record(skill_md: Path):
    try:
    ...
    if match:
        desc = match.group(1).strip().strip("\"'")[:300]
    return {
        "name": name,
        "path": str(skill_md),
        "description": desc,
...
def discover_skills(repos, codex_home: Path, extra_dirs, include_global: bool, pi_home=None):
    ...
    if include_global:
        pi_root = Path(pi_home).expanduser() if pi_home else Path.home() / ".pi" / "agent"
        roots += [
            pi_root / "skills",
    ...
    for root in roots:
        ...
        if is_pi_skill_root(root):
            files.extend(
                path for path in root.glob("*.md") if path.name != "SKILL.md"
            )
        for skill_md in sorted(set(files)):
            record = skill_record(skill_md)
            if not record or record["name"] in skills:
                continue
...
def resolve_pi_sessions_dir(pi_home: Path, sessions_dir):
    if sessions_dir:
        return Path(sessions_dir).expanduser()
    return pi_home / "sessions"


def pi_session_kind(path: Path, sessions_dir: Path) -> str:
    """Classify a JSONL path as parent, subagent, or artifact."""
    try:
    ...
    if "subagent-artifacts" in parts:
        return "artifact"
    if path.name == "session.jsonl" or len(parts) > 2:
        return "subagent"
    return "parent"


def read_pi_header(path: Path):
    try:
...
def find_pi_session_files(sessions_dir: Path, cutoff: datetime, include_subagents: bool):
    """Find recent Pi parent sessions and, optionally, nested child sessions."""
    if not sessions_dir.is_dir():
        return []
    files = []
    for path in sessions_dir.rglob("*.jsonl"):
        kind = pi_session_kind(path, sessions_dir)
        if kind == "artifact":
            continue
        ...
        try:
            ...
            continue
        if mtime >= cutoff:
            files.append((mtime, path, kind == "subagent"))
    files.sort(key=lambda item: item[0], reverse=True)
    return files


def pi_entry_timestamp(entry, message):
...
def parse_pi_session(path: Path, skill_names, include_subagents: bool, is_subagent=False):
    ...
    skills_used = set()
    first_ts = last_ts = None
    skill_invoke = re.compile(r"/skill:([a-z0-9-]+)", re.IGNORECASE)
    ...
    for line in raw.splitlines():
        ...
        if not isinstance(message, dict):
            continue
        ts = pi_entry_timestamp(obj, message)
        if ts:
            first_ts = first_ts or ts
            last_ts = ts
        ...
        if role == "user":
            ...
            stats["user_turns"] += 1
            entries.append(("user", truncate(text, MAX_MSG_CHARS)))
            for name in skill_invoke.findall(text):
...
def detect_skills_from_entries(entries, skill_names):
    tool_text = "\n".join(
        ...
        if role in ("skill", "user") or role.startswith("tool:")
    ).replace("\\", "/")
    detected = set()
...
def main():
    ...
    pi_sessions_dir = resolve_pi_sessions_dir(pi_home, args.pi_sessions_dir)
    ...
    if requested_pi and pi_sessions_dir.is_dir():
        pi_files = find_pi_session_files(
            pi_sessions_dir,
            cutoff,
        ...
        sources["pi"] = {
            "home": str(pi_home),
            "sessions_dir": str(pi_sessions_dir),
            "records_in_window": len(pi_files),
        }
        scanned_count += len(pi_files)
        for mtime, path, is_subagent in pi_files:
            header = read_pi_header(path)
            cwd = header.get("cwd") if header else None
            if not args.all_conversations and not session_matches_repos(cwd, repos):
                continue
        ...
        print(
            f"error: Pi session directory not found at {pi_sessions_dir}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not sources:
        print(
            "error: no Pi, Claude Code, Codex, or Warp session source found",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    inventory = {
        ...
        "pi_sessions_dir": str(pi_sessions_dir) if "pi" in sources else None,
