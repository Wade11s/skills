...
# Simple plus mark, inlined so the page stays self-contained. The same path
# data is redrawn on canvas for the share image.
MARK_VIEWBOX = (32, 32)
MARK_PATHS = [
    ("M14 8h4v6h6v4h-6v6h-4v-6H8v-4h6z", "#1a1522"),
]
MARK_SVG = (
    '<svg class="mark" viewBox="0 0 32 32" fill="none" aria-hidden="true" '
    'xmlns="http://www.w3.org/2000/svg">'
    '<rect x="3" y="3" width="26" height="26" stroke="currentColor" stroke-width="2"/>'
    '<path d="M14 8h4v6h6v4h-6v6h-4v-6H8v-4h6z" fill="currentColor"/>'
    "</svg>"
)

# Sticky report footer.
STAMP_NAME = "All analysis ran locally"
STAMP_SUB = "transcripts were not uploaded"

# Attribution shown only in the exported share image.
SHARE_STAMP_NAME = "Get your report with /skill:skill-doctor"
SHARE_STAMP_SUB = "local HTML report"

# White ground with a dot grid, monospace type, #2a1eff accent, hairline
# rgba(13,10,61) rules, square corners, lowercase labels, uppercase meta bars.
PAGE_CSS = """
...
def render_page(r) -> str:
    ...
    card_data = json.dumps({
        ...
        "paths": [{"d": d, "fill": fill} for d, fill in MARK_PATHS],
...
<p class="muted">Generated {esc(generated_at)} &middot; harness: {esc(r.get('harness', 'pi'))}</p>
<div class="scorecard">
