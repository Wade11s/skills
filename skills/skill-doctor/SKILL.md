---
name: skill-doctor
description: "Grades installed agent skills by scoring local conversation history (Pi sessions under ~/.pi/agent/sessions, plus Claude Code, Codex, and Warp when present) against efficiency and code-quality rubrics, then drafts skill edits and a local HTML report. Use when the user wants their agent setup graded, asks which skills are actually firing, or runs /skill:skill-doctor."
---
# skill-doctor

Grade the user's agent setup by scoring recent local conversations, then propose concrete skill edits and render one local HTML report.
...
Everything runs locally. Never upload transcripts, session files, or any excerpt of them. Do not `/share` a session that contains them. The only shareable artifact is the HTML report the user chooses to post.
...
Read `$SKILL_ROOT/references/supported-harnesses.md` and identify the harness executing this skill from the runtime context (for Pi: `PI_CODING_AGENT`, `AI_AGENT=pi`, or `PI_SESSION_FILE`). If it is unsupported or cannot be identified confidently, follow the reference's stop behavior. Do not create a report directory or read conversation history.
...
Pi has no user-question tool. Ask in the conversation. If another harness exposes a question tool, use that instead.

Skip a question whose answer is already in the triggering message. If either answer is missing, ask, then stop. Do not create `REPORT_DIR` or collect sessions until both scopes are known.
